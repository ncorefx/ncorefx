import {memoize,
        Constructor,
        Lazy,
        ConstructorGuard} from "@ncorefx/fxcore";
...
export abstract class ComposableCatalog implements IExportProvider {
     ...
     */
    @memoize()
    public async getReflectionTargets(): Promise<Set<Constructor<any>>> {
        let catalogExports = new Set<Constructor<any>>();

        await this.onEnumerate(this.processTargetOrModule.bind(this, catalogExports));
    ...
    public async getExports<T>(target: Constructor<T>): Promise<Set<ILazyExport<T>>> {
        ...
        for (let reflectionTarget of await this.getReflectionTargets()) {
            let reflectedExport = await this.reflectionContext.getExport(reflectionTarget, target, this);
     ...
     */
    protected abstract async onEnumerate(exportCallback: (targetOrModule: any) => void): Promise<void>;
     ...
     */
    private processTargetOrModule(catalogExports: Set<Constructor<any>>, targetOrModule: any): void {
        if (ConstructorGuard.isConstructor(targetOrModule)) {
            if (this.reflectionContext.meetsContext(targetOrModule)) {
                catalogExports.add(targetOrModule);
            }

            return;
        }

        for (let targetKey in targetOrModule) {
            let targetValue = targetOrModule[targetKey];

            if (ConstructorGuard.isConstructor(targetValue)) {
                if (this.reflectionContext.meetsContext(targetValue)) {
                    catalogExports.add(targetValue);
                }
            }
        }
    }
